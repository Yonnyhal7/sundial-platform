...
-- Centralize the IANA timezone contract used by direct school writes and both
-- timezone-management RPCs. Application roles do not call this helper
-- directly.
...
begin
  if not public.school_timezone_is_supported(new.timezone) then
    raise exception using
      errcode = '22023',
      message = 'Unsupported school timezone';
  end if;
  return new;
end;
$$;
...
revoke all on function public.school_timezone_is_supported(text)
  from public, anon, authenticated;
revoke all on function public.enforce_supported_school_timezone()
  from public, anon, authenticated;
