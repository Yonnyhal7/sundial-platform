...
begin
  ...
  -- Serialize imports for a school so two administrators cannot both pass the
  -- name check and create duplicate concepts concurrently.
  ...
  if exists (
    ...
    where x.id is null
       or nullif(trim(x.schedule_name), '') is null
  ) then
    return jsonb_build_object(
      'status', 'validation_error',
      'message', 'Every imported schedule must have an ID and reviewed name.'
    );
  end if;
  ...
  if exists (
    ...
    return jsonb_build_object(
      'status', 'validation_error',
      'message', 'Two imported schedules use the same ID.'
    );
  end if;
  ...
  if exists (
    ...
    from jsonb_to_recordset(coalesce(p_schedules, '[]'::jsonb))
      as x(id uuid, temp_id text, schedule_name text, schedule_type text, calendar_color text, setup_status text)
    join public.schedules existing on existing.id = x.id
  ) then
    return jsonb_build_object(
      'status', 'schedule_conflict',
      'message', 'An imported schedule ID is already in use.'
    );
  end if;
