...
export default function SchoolPublicNav({ school, schoolName, logoUrl, base, schoolDefaultAppearance }: Props) {
  ...
  return <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/90 text-slate-950 shadow-sm backdrop-blur-xl dark:border-white/10 dark:bg-[#141618]/90 dark:text-white">
    <div className="mx-auto flex min-h-20 max-w-[1360px] items-center gap-5 px-5 sm:px-8 lg:px-12">
      <Link href={base || "/"} className="flex min-w-0 items-center gap-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-[var(--school-primary)]"><SchoolLogo schoolName={schoolName} logoUrl={logoUrl} variant="websiteHeader" className="h-11 w-11 p-1" /><span className="max-w-[13rem] truncate text-sm font-black sm:max-w-[17rem]">{schoolName}</span></Link>
    ...
    {open && <nav id="public-mobile-menu" aria-label="Mobile" className="border-t border-slate-200 px-5 py-2 dark:border-white/10 xl:hidden"><div className="mx-auto grid max-w-[1360px] gap-0.5">{items.map(([label, href]) => <Link key={href} href={href} aria-current={active(href) ? "page" : undefined} className={`flex min-h-11 items-center rounded-xl px-4 text-sm font-bold ${active(href) ? "bg-[var(--school-primary)] text-[var(--school-primary-text)]" : "hover:bg-slate-100 dark:hover:bg-white/10"}`}>{label}</Link>)}</div></nav>}
  </header>;
}
