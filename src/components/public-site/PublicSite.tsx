...
export function PublicSiteFooter({ school, base }: { school: { name: string; logo_url: string | null; district_name: string | null; address: string | null; phone_number: string | null; main_office: string | null }; base: string }) {
  return <footer className="border-t border-slate-200 bg-slate-950 py-12 text-slate-300 dark:border-white/10"><PublicContainer className="grid gap-10 md:grid-cols-[1.3fr_1fr_1fr]">
    <div className="flex gap-4"><SchoolLogo schoolName={school.name} logoUrl={school.logo_url} size="lg" /><div><p className="text-lg font-black text-white">{school.name}</p>{school.district_name && <p className="mt-1 text-sm">{school.district_name}</p>}<p className="mt-4 whitespace-pre-line text-sm leading-6">{school.address || "Contact the school office for location information."}</p></div></div>
    <div><p className="font-bold text-white">Contact</p>{school.phone_number && <a className="mt-3 block text-sm hover:text-white" href={`tel:${school.phone_number}`}>{school.phone_number}</a>}{school.main_office && <p className="mt-2 text-sm">Main office: {school.main_office}</p>}</div>
    <div><p className="font-bold text-white">Explore</p><nav aria-label="Footer" className="mt-3 grid gap-2 text-sm"><Link href={`${base}/announcements`}>Announcements</Link><Link href={`${base}/events`}>Events</Link><Link href={`${base}/resources`}>Resources</Link><Link href={`${base}/schedule`}>Calendar</Link><Link href={`${base}/app`}>School App</Link></nav><p className="mt-6 text-xs text-slate-500">Powered by Sundial</p></div>
