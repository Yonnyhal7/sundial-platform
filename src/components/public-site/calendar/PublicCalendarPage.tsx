...
function DayCell({ date, day, currentMonth, today, selected, academicYear, onSelect }: {
  ...
  const classes = `relative flex aspect-square min-h-11 min-w-0 flex-col items-start overflow-hidden rounded-xl border p-1.5 text-left transition sm:aspect-auto sm:min-h-20 sm:p-2 ${statusClass} ${selected ? "ring-2 ring-[var(--school-primary)] ring-offset-2 dark:ring-offset-[#101214]" : ""} ${!inCurrentMonth ? "opacity-40" : ""} ${!meaningfulDay && (isWeekend || outsideYear) ? "opacity-35" : ""}`;
...
function SelectedDayPanel({ day }: { day: PublicCalendarDay | null }) {
  if (!day) return <aside className="rounded-[1.75rem] border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-[#1b1e21] sm:p-5 lg:sticky lg:top-28"><p className="text-sm font-semibold text-slate-500 dark:text-slate-400">Select a published calendar date to view its details.</p></aside>;
  return <aside aria-live="polite" className="rounded-[1.75rem] border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-[#1b1e21] sm:p-5 lg:sticky lg:top-28">
    <p className="text-xs font-black uppercase tracking-[.18em] text-[var(--school-primary)]">Selected day</p>
    <h2 className="mt-2 text-xl font-black tracking-tight sm:text-[1.375rem]">{formatFullDate(day.date)}</h2>
    {day.isSchoolDay === false ? <div className="mt-4 rounded-2xl border border-rose-200 bg-rose-50 p-3.5 dark:border-rose-900/60 dark:bg-rose-950/25"><p className="font-black text-rose-800 dark:text-rose-100">No School</p><p className="mt-1 text-sm text-rose-700 dark:text-rose-200">{day.label || "School is closed."}</p></div> : <>
      ...
      {!day.scheduleName ? <p className="mt-4 rounded-2xl bg-slate-100 p-3.5 text-sm text-slate-600 dark:bg-white/[.06] dark:text-slate-300">A bell schedule has not been assigned for this date.</p> : day.scheduleSetupStatus === "needs_times" || day.periods.length === 0 ? <p className="mt-4 rounded-2xl bg-slate-100 p-3.5 text-sm text-slate-600 dark:bg-white/[.06] dark:text-slate-300">Bell times have not been published yet.</p> : <div className="mt-4 space-y-1.5">{day.periods.map((period) => <div key={period.id} className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 px-3 py-2.5 dark:bg-white/[.05]"><span className="min-w-0 truncate text-sm font-bold">{period.name}</span><span className="shrink-0 text-xs font-semibold text-slate-500 dark:text-slate-400 sm:text-sm">{formatPeriodTime(period.start_time)} – {formatPeriodTime(period.end_time)}</span></div>)}</div>}
    </>}
    {day.events.length > 0 && <div className="mt-5 border-t border-slate-200 pt-4 dark:border-white/10"><h3 className="text-sm font-black">School events</h3><div className="mt-2.5 space-y-1.5">{day.events.map((event) => <div key={event.id} className="rounded-xl bg-sky-50 p-2.5 dark:bg-sky-950/25"><p className="text-sm font-bold">{event.title}</p><p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{event.startTime ? formatPeriodTime(event.startTime) : "All day"}{event.location ? ` · ${event.location}` : ""}</p></div>)}</div></div>}
  </aside>;
}
...
export default function PublicCalendarPage({ calendar }: { calendar: PublicCalendarViewModel }) {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid items-start gap-4 lg:grid-cols-[minmax(0,2.15fr)_minmax(17rem,.85fr)]">
      <section aria-label={`${monthLabel(visibleMonth)} calendar`} className="min-w-0 rounded-[1.75rem] border border-slate-200 bg-white p-2 shadow-sm dark:border-white/10 dark:bg-[#1b1e21] sm:p-4">
        <div className="grid grid-cols-7 gap-1 sm:gap-1.5">{weekdays.map(([narrow, wide], index) => <div key={`${wide}-${index}`} className="py-1.5 text-center text-[11px] font-black uppercase tracking-wide text-slate-400"><span className="sm:hidden">{narrow}</span><span className="hidden sm:inline">{wide}</span></div>)}</div>
        <div className="grid grid-cols-7 gap-1 sm:gap-1.5">{gridDates.map((date) => <DayCell key={date} date={date} day={dayByDate.get(date)} currentMonth={visibleMonth} today={calendar.today} selected={selectedDate === date} academicYear={calendar.academicYear} onSelect={setSelectedDate} />)}</div>
      </section>
