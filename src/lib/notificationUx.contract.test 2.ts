...
describe("notification UX integration contracts", () => {
  ...
  it("keeps create and index navigation distinct", () => {
    const index = read("src/app/[school]/admin/notifications/page.tsx");
    const create = read("src/app/[school]/admin/notifications/new/page.tsx");
    const back = read("src/components/admin/NotificationBackButton.tsx");

    expect(create).toContain(
      'const notificationsHref = `${await getSchoolAdminPath(school)}/notifications`;'
    );
    expect(create).toContain("fallbackHref={notificationsHref}");
    expect(create).not.toContain("fallbackHref={`${notificationsHref}/new`}");
    expect(back).toContain("Back to Notifications");
    expect(back).toContain("canUseNotificationsIndexHistory");
    expect((index.match(/Create notification/g) || [])).toHaveLength(1);
  });
