...
export default async function NotificationsPage({ params, searchParams }: { params: Promise<{ school: string }>; searchParams: Promise<{ view?: string }> }) {
  ...
  return <main className="mx-auto max-w-6xl px-6 py-8 text-slate-950 dark:text-white">
    ...
    <section className="mt-5 overflow-hidden rounded-2xl border bg-white dark:border-[#3a3a3a] dark:bg-[#242424]">{campaigns?.length ? campaigns.map((campaign) => {
      ...
      return <Link href={`${base}/${campaign.id}`} key={campaign.id} className="flex items-center justify-between gap-4 border-b p-5 last:border-0 dark:border-[#3a3a3a]"><div><h2 className="font-bold">{campaign.title}</h2><p className="mt-1 line-clamp-1 text-sm text-slate-500">{campaign.body}</p><p className="mt-2 text-xs text-slate-500">{formatTimestampInTimeZone(campaign.created_at, timeZone)}</p></div><div className="text-right"><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold dark:bg-[#333]">{getCampaignStatusLabel(displayStatus)}</span>{summary && <p className="mt-2 whitespace-pre-line text-xs text-slate-500">{summary}</p>}</div></Link>;
